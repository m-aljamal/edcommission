    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-1">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 mb-6 sticky top-24">
          <h3 className="text-xl font-bold text-[#203441] mb-4"> {t.project.projectDetails}  </h3>

          <div className="space-y-4">
            <div className="flex items-center">
              <MapPin className="w-5 h-5 text-[#476c86] mt-0.5 mx-3" />
              <div>
                <p className="text-gray-500 text-sm">{t.project.location}</p>
                <p className="font-medium">{project.location}</p>
              </div>
            </div>

            <div className="flex items-center">
              <Users className="w-5 h-5 text-[#476c86] mt-0.5 mx-3" />
              <div>
                <p className="text-gray-500 text-sm">{t.tabs.beneficiaries}</p>
                <p className="font-medium">{project.beneficiaries.toLocaleString()}</p>
              </div>
            </div>

            <div className="flex items-center">
              <Calendar className="w-5 h-5 text-[#476c86] mt-0.5 mx-3" />
              <div>
                <p className="text-gray-500 text-sm">{t.project.startDate}</p>
                <p className="font-medium">{new Date(project.startDate).toDateString()}</p>
              </div>
            </div>
            ...
            <div className="flex items-center">
              <Calendar className="w-5 h-5 text-[#476c86] mt-0.5 mx-3" />
              <div>
                <p className="text-gray-500 text-sm">{t.project.endDate}</p>
                {project.status === 'in-progress' ? (
                  <p>{t.project.inProgress}</p>
                ) : (
                  <p className="font-medium">{new Date(project.endDate).toDateString()}</p>
                )}
              </div>
            </div>
            {project.budget !== 0 && (
              <div className="flex items-center">
                <BarChart3 className="w-5 h-5 text-[#476c86] mt-0.5 mx-3" />
                <div>
                  <p className="text-gray-500 text-sm">{t.project.budget}</p>
